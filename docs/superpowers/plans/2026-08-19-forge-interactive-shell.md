...
Note the ref. `useInput` handlers run synchronously but `setState` does not, so state cannot be the source of truth for input handling — a burst of `DOWN DOWN ENTER` in one write would otherwise submit a stale item while the frame shows the new one. Any later component that reads its own state inside a `useInput` handler needs this same shape.
